...
void tester(List<num> signature, int n, List<num> answer) {
  List<num> result = tribonacci(signature, n);
  bool correct = '$answer' == '$result';
  print(' $correct ${correct ? '☑ Test Passed' : '$result is no $answer'}');
}
/* my solution */
List<num> tribonacci1(List<num> s, int n) {
  // var newValue = signature.getRange(signature.length, signature.length-2).fold(0, (previousValue, element) =>  previousValue + element);
  while (s.length<n) {
    var newValue = s[s.length - 1] + s[s.length - 2] + s[s.length - 3];
    s.add(newValue);
  }
